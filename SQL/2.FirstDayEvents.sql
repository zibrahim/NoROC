...
drop materialized view if exists NoROC.allevents cascade; 
create materialized view NoROC.allevents as
select labevents.subject_id, 
		...
		from labevents left join icustays 
				...
				where labevents.itemid in(select distinct itemid from NoROC.all_items)
...
select 	outputevents.subject_id, 
		...
		from outputevents left join icustays 
				...
				where outputevents.itemid in(select distinct itemid from NoROC.all_items)
...
select 	inputevents_cv.subject_id, 
		...
		from inputevents_cv left join icustays 
				...
				where inputevents_cv.itemid in(select distinct itemid from NoROC.all_items)
...
select 	inputevents_mv.subject_id, 
		...
		from inputevents_mv left join icustays 
				...
				where inputevents_mv.itemid in(select distinct itemid from NoROC.all_items)
...
select 	chartevents.subject_id, 
		...
		from chartevents left join icustays 
				...
				where chartevents.itemid in(select distinct itemid from NoROC.all_items)
...
select 	procedureevents_mv.subject_id, 
		...
		from procedureevents_mv left join icustays 
				...
				where procedureevents_mv.itemid in(select distinct itemid from NoROC.all_items)
